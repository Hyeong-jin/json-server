...
module.exports = (db, name, opts) => {
  ...
  function list(req, res, next) {
    ...
    if (_page) {
      ...
      const pagenation = {
        ...
        first: page.current === page.first,
        last: page.current === page.last,
      };
      res['pagenation'] = pagenation;
  ...
  function destroy(req, res, next) {
    ...
    if (opts._isFake) {
      ...
      const singularResource = pluralize.singular(name);
      opts.parentColumn = `${singularResource}${opts.foreignKeySuffix}`;
      opts.parentId = req.params.id;
      // Remove dependents documents
      const removable = db._.getRemovable(db.getState(), opts);
      //{"name":"nodes","id":"79DE087B-1FF8-4AE4-8B75-D81446D20D44"}
      removable.forEach((item) => {
        // console.log('NAME : ', item.name, 'ID : ', item.id, JSON.stringify(item));
